...
class LambdaExpressions : FunctionLearn {
    override fun showResult() {
        ...
        // Также лямбда-выражение можно выполнить сразу при определении с помощью скобочек ()
        ...
        val helloUnit = { println("HelloUnit") } // Нет входящих, выход Unit - эти записи опускаем. Остается тело выполнения.
        ...
        val sumType // имя переменной
        : (Int, Int) -> Int // тип переменной или сигнатура лямбды: что на входе, что на выходе
                = {            // Лямбда заключается в {}
                x: Int, y: Int // Параметры принимаемые на входе
            ->
            x + y              // Тело лямбда-функции
                }
        ...
        // Если лямбда-выражение не принимает никаких параметров, то указываются пустые скобки:
                                                                                    // (типы_параметров) -> возвращаемый_тип
        val l1 : () -> String = { "Result l1: () -> String" }
        // Постоянная по имени l1 у которой тип функции () -> String и присвоено значение {Result l1: () -> String"} - это лябда, котрая возвращает последнее значение, и это строковый литерал

        println(l1()) // Выполняем лямбду сохраненную в постоянную. Результат String - на консоль выводим

        val l2 : String = { "hello" }()       // Постоянная по имени l2 тип String, которой присвоено выполнение лямбды {последнее значение в скобках}, запуск через ()
        ...
        // В Kotlin существует соглашение: если последний параметр функции является функцией, то лямбда-выражение, переданное в качестве соответствующего аргумента, может быть заключено в скобки:
         val product = items.fold(1) { // Параметр, который лямбда, вынесли из (), так как он последний
                acc, e -> acc * e
        }

        //Когда лямбда-выражение является единственным аргументом функции, круглые скобки могут быть опущены:
        run { println("...") }
        // Анонимная, нет имени, простая сигнатура, опущена, единственный параметр, () опущены - выполнение тела лямбды через run
